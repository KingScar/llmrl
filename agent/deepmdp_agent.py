...
class DeepMDPAgent(object):
    ...
    def __init__(
        ...
        #self.vlm = Qwen2VLForConditionalGeneration.from_pretrained(v_url, torch_dtype="auto", device_map=device)
        #self.v_processor = AutoProcessor.from_pretrained(v_url)
    ...
    def contrastive_loss_forward(self,
        ...
        hidden1 = hidden1.view(batch_size, -1)
        hidden2 = hidden2.view(batch_size, -1)
    ...
    def update_reward_f(self, obs, knows, action, next_obs, reward, L, step):

        h, h3,h1, ka = self.critic.encoder(obs, knows, fusion=False)
        # import pdb
        # pdb.set_trace()
        ...
        next_h,next_h3,next_h1 = self.critic.encoder(next_obs, knows, fusion=False)
        ...
        con_loss = self.contrastive_loss_forward(h1, ka)

        L.log('train_ae/con_loss3', con_loss, step)
        # L.log('train_ae/curl_loss4', curl_loss4, step)


        # total_loss = total_loss + 0.01*(curl_loss1+curl_loss3)
        total_loss = total_loss + 0.05*(con_loss)
    ...
    def update_critic_f(self, obs, action, knows, reward, next_obs, not_done, L, step):
        with torch.no_grad():
            _, policy_action, log_pi, _ = self.actor(next_obs, knows)
            target_Q1, target_Q2 = self.critic_target(next_obs, policy_action, knows)
            target_V = torch.min(target_Q1, target_Q2) - self.alpha.detach() * log_pi
            target_Q = reward + (not_done * self.discount * target_V)
    ...
    def update_actor_and_alpha_f(self, obs, knows, L, step):
        # detach encoder, so we don't update it with the actor loss
        _, pi, log_pi, log_std = self.actor(obs, knows, detach_encoder=True)
        actor_Q1, actor_Q2 = self.critic(obs, pi, knows, detach_encoder=True)
    ...
    def update_se(self, replay_buffer, L, step):
 
        

        obs, action, _, reward, next_obs, not_done, knows = replay_buffer.sample_dm3dp()
        ...
        self.update_critic_f(obs, action, knows, reward, next_obs, not_done, L, step)
        self.update_reward_f(obs, action, knows, next_obs, reward, L, step)

        if step % self.actor_update_freq == 0:
            self.update_actor_and_alpha_f(obs, knows, L, step)
