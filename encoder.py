...
class PixelEncoderCarla098_SNN(nn.Module):
    """Convolutional encoder of pixels observations."""
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32, stride=None):
        super().__init__()
        ...
        self.feature_dim = feature_dim
        self.num_layers = num_layers
        self.dvs_obs_shape = obs_shape

        self.dvs_head_convs = nn.ModuleList()
        self.dvs_head_skips = nn.ModuleList()
        self.dvs_head_act = nn.ModuleList()
        self.dvs_head_pool = nn.ModuleList()

        self.dvs_head_convs.append(snn.Conv2d(1, 32, kernel_size=5, stride=2, bias=False))  # input: 15
        self.dvs_head_skips.append(snn.Conv2d(1, 32, kernel_size=1, bias=False))
        self.dvs_head_act.append(neuron.IFNode(surrogate_function=surrogate.ATan()))
        self.dvs_head_pool.append(snn.MaxPool2d(5, 2))

        self.dvs_head_convs.append(snn.Conv2d(32, 64, kernel_size=3, stride=2, bias=False))
        self.dvs_head_skips.append(snn.Conv2d(32, 64, kernel_size=1, bias=False))
        self.dvs_head_act.append(neuron.IFNode(surrogate_function=surrogate.ATan()))
        self.dvs_head_pool.append(snn.MaxPool2d(3, 2))

        functional.set_step_mode(self, step_mode='m')
        functional.set_backend(self, backend='cupy')

        out_dim = 6
        self.fc = nn.Linear(64 * out_dim * out_dim, self.feature_dim)
        self.ln = nn.LayerNorm(self.feature_dim)
    ...
    def forward_conv(self, obs, vis=False):
        ...
        if self.dvs_obs_shape[0] == 5 * 3:  # width * height * 15
            nonzero_ev = (obs != 0)
            num_nonzeros = nonzero_ev.sum()
            if num_nonzeros > 0:
                # compute mean and stddev of the **nonzero** elements of the event tensor
                ...
                mask = nonzero_ev.float()
                if stddev != 0:
                    dvs_obs = mask * (obs - mean) / stddev
            # pass
        elif self.dvs_obs_shape[0] == 2 * 3:
            obs = obs / 255.

        # obs: 1, 15, 128, 128
        dvs_snn_obs = einops.rearrange(
            obs, "n t h w -> t n 1 h w"
        )
        # dvs_snn_obs: ([15, 1, 1, 128, 128])
        dvs_conv1 = self.dvs_head_convs[0](dvs_snn_obs)
        dvs_conv1 = self.dvs_head_act[0](dvs_conv1)
        dvs_conv1 = self.dvs_head_pool[0](dvs_conv1)
        # print("dvs_conv1:", dvs_conv1.shape)
        dvs_conv1 = dvs_conv1 + self.dvs_head_pool[0](self.dvs_head_pool[0](self.dvs_head_skips[0](dvs_snn_obs)))
        self.outputs['dvs_conv1'] = dvs_conv1.mean(0)  # ([15, B, 32, 62, 62])
        # print("dvs_conv1:", dvs_conv1.shape)

        dvs_conv2 = self.dvs_head_convs[1](dvs_conv1)
        dvs_conv2 = self.dvs_head_act[1](dvs_conv2)
        dvs_conv2 = self.dvs_head_pool[1](dvs_conv2)
        # print("dvs_conv2:", dvs_conv2.shape)
        dvs_conv2 = dvs_conv2 + self.dvs_head_pool[1](self.dvs_head_pool[1](self.dvs_head_skips[1](dvs_conv1)))
        self.outputs['dvs_conv2'] = dvs_conv2.mean(0)  # ([15, B, 64, 30, 30])
        # print("dvs_conv2:", dvs_conv2.shape)
        dvs_conv2 = dvs_conv2.mean(0)  # # ([B, 128, 6, 6])

        if vis:
            return dvs_conv2
        else:
            h = dvs_conv2.view(dvs_conv2.size(0), -1)
            return h
    ...
    def copy_conv_weights_from(self, source):
        ...
        for i in range(2):
            ...
            tie_weights(src=source.dvs_head_skips[i], trg=self.dvs_head_skips[i])
...
class DMR_SNN(nn.Module):
    ...
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32, stride=1):
        ...
        self.rgb_head_convs = nn.ModuleList()
        self.rgb_head_convs.append(nn.Conv2d(3*3, 64, 5, stride=2))   # input: 9
        self.rgb_head_convs.append(nn.Conv2d(64, 128, 3, stride=2))
        self.rgb_head_convs.append(nn.Conv2d(128, 256, 3, stride=2))
        self.rgb_head_convs.append(nn.Conv2d(256, 256, 3, stride=2))
        self.rgb_fc = nn.Linear(256 * out_dims, self.feature_dim)
        ...
        # self.dvs_head_convs.append(snn.Conv2d(64, 128, kernel_size=3, stride=2, bias=False))
        # self.dvs_head_skips.append(snn.Conv2d(64, 128, kernel_size=1, bias=False))
        # self.dvs_head_act.append(neuron.IFNode(surrogate_function=surrogate.ATan()))
        # self.dvs_head_pool.append(snn.MaxPool2d(3, 2))

        # self.dvs_head_convs.append(snn.Conv2d(256, 256, kernel_size=3, stride=2, bias=False))
        # self.dvs_head_act.append(neuron.IFNode(surrogate_function=surrogate.ATan()))
        self.dvs_fc = nn.Linear(64 * out_dims, self.feature_dim)
        self.dvs_ln = nn.LayerNorm(self.feature_dim)
        # self.dvs_Q = nn.Linear(256 * out_dims, self.feature_dim)
    ...
    def forward_conv(self, obs):
        ...
        rgb_conv = torch.relu(self.rgb_head_convs[0](rgb_obs))
        self.outputs['rgb_conv1'] = rgb_conv    # ([1, 64, 62, 62])
        rgb_conv = torch.relu(self.rgb_head_convs[1](rgb_conv))
        self.outputs['rgb_conv2'] = rgb_conv    # ([1, 128, 30, 30])
        rgb_conv = torch.relu(self.rgb_head_convs[2](rgb_conv))
        self.outputs['rgb_conv3'] = rgb_conv    # ([1, 256, 14, 14])
        rgb_conv = torch.relu(self.rgb_head_convs[3](rgb_conv))
        self.outputs['rgb_conv4'] = rgb_conv    # ([1, 256, 6, 6])


        # dvs_obs: ([1, 15, 128, 128])
        dvs_snn_obs = einops.rearrange(
            dvs_obs, "n t h w -> t n 1 h w"
        )
        # dvs_snn_obs: ([15, 1, 1, 128, 128])
        dvs_conv1 = self.dvs_head_convs[0](dvs_snn_obs)
        dvs_conv1 = self.dvs_head_act[0](dvs_conv1)
        ...
        # dvs_conv3 = self.dvs_head_convs[2](dvs_conv2)
        # dvs_conv3 = self.dvs_head_act[2](dvs_conv3) # ([15, B, 128, 14, 14])
        # # print("dvs_conv3:", dvs_conv3.shape)
        # dvs_conv3 = self.dvs_head_pool[2](dvs_conv3)    # ([15, B, 128, 6, 6])
        # # print("dvs_conv3:", dvs_conv3.shape)
        # dvs_conv3 = dvs_conv3 + self.dvs_head_pool[2](self.dvs_head_pool[2](self.dvs_head_skips[2](dvs_conv2)))
        # # print("dvs_conv3:", dvs_conv3.shape)
        # self.outputs['dvs_conv3'] = dvs_conv3   # ([15, B, 128, 6, 6])
        # dvs_conv3 = dvs_conv3.mean(0)           # # ([B, 128, 6, 6])
        # print("dvs_conv3:", dvs_conv3.shape)

        # dvs_conv4 = self.dvs_head_convs[3](dvs_conv3)
        # dvs_conv4 = self.dvs_head_act[3](dvs_conv4)
        # # dvs_conv4 = dvs_conv4 + dvs_conv3
        # self.outputs['dvs_conv4'] = dvs_conv4   # ([15, B, 256, 6, 6])
        # dvs_conv4 = dvs_conv4.squeeze(1)
        # print("dvs_conv4.shape:", dvs_conv4.shape)
        # dvs_conv4 = dvs_conv4.mean(0)           # # ([B, 256, 6, 6])
        ...
        return rgb_conv, dvs_conv2, con_conv
...
class ChannelAttention_s(nn.Module):
    """
...
class Attention_s(nn.Module):
    ...
    def forward(self, ka, xt, x):
        m_batchsize, C, height, width = ka.size()

        proj_query = self.query_conv(ka).view(m_batchsize, -1, width * height).permute(0, 2, 1)
        proj_key = self.key_conv(xt).view(m_batchsize, -1, width * height)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)

        proj_query2 = self.query_conv(ka).view(m_batchsize, -1, width * height).permute(0, 2, 1)
        proj_key2 = self.key_conv2(x).view(m_batchsize, -1, width * height)
        energy2 = torch.bmm(proj_query2, proj_key2)
        attention2 = self.softmax2(energy2)

        attention = self.softmax(attention + attention2)
        
        proj_value = self.value_conv(xt).view(m_batchsize, -1, width * height)
        proj_value2 = self.value_conv2(x).view(m_batchsize, -1, width * height)
        ...
        attention_t = outt.view(m_batchsize, -1, C*height*width)
        attention_t = self.fn(attention_t)
        attention_t = outt.view(m_batchsize, C, height, width)
        ...
        attention_x = outx.view(m_batchsize, -1, C*height*width)
        attention_x = self.fn2(attention_x)
        attention_x = outt.view(m_batchsize, C, height, width)

        print(attention_x.shape)
        print(x.shape)
    ...
    def forward_s(self, x1,x2,x3,xt):
        """
            inputs :
...
class DualEncoder(nn.Module):
    ...
    def forward_conv2(self, x):
        ...
        conv = torch.relu(self.convs2[0](x))
        self.outputs1['conv1'] = conv

        for i in range(1, self.num_layers):
            conv = torch.relu(self.convs2[i](conv))
            self.outputs1['conv%s' % (i + 1)] = conv
    ...
    def forward(self, obs, knows, detach=False,fusion=True, training=True):
        diff  = torch.cat([obs[:,6:9,:,:]-obs[:,3:6,:,:],obs[:,3:6,:,:]-obs[:,0:3,:,:]],dim=1)
        h = self.forward_conv(diff)

        h1 = self.forward_conv1(obs[:,0:3,:,:])
        

        ka = self.forward_conv2(knows)

        if training == True:
            _, h1, h = self.att_qkv(ka, h1, h)
        else:
            _, h1, h = self.att_qkv(ka, h1, h)
...
class Semotion(DualEncoder):
    ...
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32, stride=1):
        ...
        for i in range(num_layers - 1):
            self.convs2.append(nn.Conv2d(num_filters, num_filters, 3, stride=stride))


        out_dims = 39  # if defaults change, adjust this as needed
        self.fc = nn.Linear(119072, self.feature_dim)
        self.ln = nn.LayerNorm(self.feature_dim)

        self.fc1 = nn.Linear(119072, self.feature_dim)
        ...
        self.att_qkv = Attention_s(num_filters)
        ...
        self.selfatt_video = SelfAttention_s(num_filters)
        self.selfatt_image = SelfAttention_s(num_filters)
...
_AVAILABLE_ENCODERS = {'pixel': PixelEncoder,
                       ...
                       'pixelCarla098_SNN': PixelEncoderCarla098_SNN,
                       'pixelCon': pixelCon,
                       'DMR_CNN': DMR_CNN,
                       'DMR_SNN': DMR_SNN,
