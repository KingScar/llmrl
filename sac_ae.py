...
class Actor(nn.Module):
    ...
    def forward(
        self, obs, ka=torch.zeros(3,128,128), compute_pi=True, compute_log_pi=True, detach_encoder=False
    ):
        final_fea, _ = self.encoder(obs, ka, detach=detach_encoder)
...
class Critic(nn.Module):
    ...
    def forward(self, obs, action, ka=torch.zeros(3,128,128), detach_encoder=False):
