...
FORMAT_CONFIG = {
    'rl': {
        'train': [('episode', 'E', 'int'), ('step', 'S', 'int'),
                  ('duration', 'D', 'time'), ('episode_reward', 'R', 'float'),
                  ('batch_reward', 'BR', 'float'),
                  ('actor_loss', 'ALOSS', 'float'),
                  ('critic_loss', 'CLOSS', 'float'),
                  ('ae_loss', 'RLOSS', 'float'), ('max_rat', 'MR', 'float')],
...
class Logger(object):
    ...
    def _try_sw_logs(self, key, value, step):
        if self._sw is not None:
            self._sw.add_scalars(key, value, step)
    ...
    def logs(self, key, value, step, n=1):
        assert key.startswith('train') or key.startswith('eval')

        self._try_sw_logs(key, value, step)
        mg = self._train_mg if key.startswith('train') else self._eval_mg
